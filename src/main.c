...
#else
# ifdef freebsd
int apm_read(struct apm_reply reply);
# endif
#endif
...
static void draw_timedigit(ApmInfos infos) {
    ...
    */
    ...
    time_left = (infos.ac_line_status) ? infos.battery_time / 60 : infos.battery_time;
#endif
...
static void apm_getinfos(ApmInfos *infos) {
    struct apm_reply temp_info;

/* straight out of apm.c */
    ...
    struct sockaddr_un s_un;
        ...
        strlcpy(s_un.sun_path, APMDEV, sizeof(s_un.sun_path));
       ...
       if (send(sock, &cmd, sizeof(cmd), 0) == sizeof(cmd)) {
                if (recv(sock, &temp_info, sizeof(temp_info), 0) != sizeof(temp_info)) {
                        warn("invalid reply from APM daemon");
			infos->ac_line_status = 0;
			infos->battery_status = 0;
			infos->battery_percentage = 0;
			infos->battery_time = 0;
			infos->using_minutes = 1;
			return;
                }
        } else {
    ...
    infos->ac_line_status     = temp_info.batterystate.ac_state;
    infos->battery_status     = temp_info.batterystate.battery_state;
    infos->battery_percentage = temp_info.batterystate.battery_life;
    infos->battery_time       = temp_info.batterystate.minutes_left;
    infos->using_minutes      = 1;
}
...
#else
# ifdef freebsd
/* straight out of src/usr.sbin/apm.c:open_socket(const char *sockname) */
int apm_read(struct apm_reply temp_info) {
    int sock, errr;
    struct sockaddr_un s_un;
